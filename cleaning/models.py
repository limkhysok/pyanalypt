...
class CleaningOperation(models.Model):
    ...
    dataset = models.ForeignKey(Dataset, on_delete=models.CASCADE, related_name="cleaning_ops")
    issue = models.ForeignKey(Issue, on_delete=models.SET_NULL, null=True, blank=True, related_name="cleaning_ops")
    ...
    def __str__(self):
        return f"{self.operation_type} on {self.column_name or 'dataset'} ({self.status})"
